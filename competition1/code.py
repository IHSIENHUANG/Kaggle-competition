...
print('load train....')
# we save only day 9
train_df = pd.read_csv(path+"train_sample2.csv", dtype=dtypes,skiprows = range(1, 1000000),usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
print('load test....')
...
print('hour, day, wday....')#get the time of hour day and wday
...
print (train_df.head(10))# check the status 
...
#find out the max num of training and testing
...
def get_keras_data(dataset):#make all of them become a one dimentional array
    X = {
...
fe = concatenate([(emb_app), (emb_ch), (emb_dev), (emb_os), (emb_h), (emb_d), (emb_wd), (emb_qty), (emb_c1), (emb_c2)])# use concatenate to combine these array
s_dout = SpatialDropout1D(0.2)(fe)# try different parameter of dropout
fl1 = Flatten()(s_dout)
...
model.fit(train_df, y_train, batch_size=batch_size, epochs=10, class_weight=class_weight, shuffle=True, verbose=2)
...
print("predicting....")
sub['is_attributed'] = model.predict(test_df, batch_size=batch_size, verbose=1)#verbose show the status 0 for no 1 for process 2 for the whole line
del test_df; gc.collect()
print("writing....")
